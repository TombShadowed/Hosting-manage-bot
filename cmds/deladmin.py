...
def init(app, manager, OWNER_IDS):
    ...
    async def del_admin_cmd(client, message):
        ...
        if len(args) < 2:
            await message.reply_text('<b><blockquote>◉ 𝗨𝗦𝗔𝗚𝗘: /deladmin [telegram_id]</blockquote></b>')
            return
        try:
            tid = int(args[1])
            manager.storage.del_admin(tid)
            await message.reply_text(f'<b><blockquote>✓ 𝗥𝗘𝗠𝗢𝗩𝗘𝗗 𝗔𝗗𝗠𝗜𝗡: {tid}</blockquote></b>')
